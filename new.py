...
def fetch_alternate_article(original_query):
    """Fetch an alternate opinion article by modifying the search query."""
    alternative_query = original_query + " alternative opinion"
    alternative_df = fetch_articles(alternative_query, num_results=1)
    if not alternative_df.empty:
        return alternative_df.iloc[0]
    return None
...
def clean_text(text):
    """Remove illegal characters that cannot be stored in Excel."""
    if isinstance(text, str):
        return re.sub(r'[\x00-\x1F\x7F-\x9F]', '', text)  # Remove control characters
    return text  # Return non-string values unchanged

def download_as_excel(dataframe):
    """Prepare dataframe for download as Excel."""
    output = BytesIO()
    cleaned_df = dataframe.applymap(clean_text)
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        cleaned_df.to_excel(writer, index=False, sheet_name='Articles')
        ...
        if articles_df.empty:
            ...
            for _, row in articles_df.iterrows():
                with st.container():
                    ...
                    alt_article = fetch_alternate_article(query)
                    if alt_article is not None:
